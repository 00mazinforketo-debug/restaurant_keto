...
import { defaultLocale, findFixedLoginAccountByPin, fixedLoginAccounts, ketoCatalogCategories, ketoCatalogMenuItems, socketEvents } from "@ros/shared";
const port = 4001;
...
const webOrigins = Array.from(new Set([process.env.WEB_ORIGIN, ...localWebOrigins].filter(Boolean)));
const localHosts = new Set(["127.0.0.1", "localhost", "::1"]);
const privateIpPattern = /^(10(?:\.\d{1,3}){3}|192\.168(?:\.\d{1,3}){2}|172\.(?:1[6-9]|2\d|3[01])(?:\.\d{1,3}){2})$/;
const authCookies = { access: "ros_access", refresh: "ros_refresh", csrf: "ros_csrf" };
...
const users = fixedLoginAccounts.map((account) => ({ ...account, isActive: true, createdAt: nowIso() }));
const categories = ketoCatalogCategories.map((category) => ({
    id: category.id,
    slug: category.slug,
...
const menuItems = ketoCatalogMenuItems.map((item) => ({
    id: item.id,
    slug: item.slug,
...
const pushActivity = (actorName, actorRole, action, entityType) => {
...
const isWithinRange = (value, startAt, endAt) => {
    const timestamp = new Date(value).getTime();
    return timestamp >= startAt.getTime() && timestamp <= endAt.getTime();
};
const isLocalDevHost = (hostname) => localHosts.has(hostname) || privateIpPattern.test(hostname);
...
const getRevenueRangeSummary = (startAt, endAt) => {
...
const getFilteredMenuItems = (params) => {
    ...
    return menuItems
        ...
        return item.translations.some((translation) => `${translation.name} ${translation.description}`.toLowerCase().includes(normalizedQuery));
    });
};
const accessCookieOptions = { httpOnly: true, sameSite: "lax", path: "/", maxAge: accessCookieMaxAgeMs };
const refreshCookieOptions = { httpOnly: true, sameSite: "lax", path: "/", maxAge: refreshCookieMaxAgeMs };
const publicCookieOptions = { sameSite: "lax", path: "/", maxAge: refreshCookieMaxAgeMs };
export const startDemoServer = (portArg = port) => {
    ...
    const isSessionIdle = (lastActiveAt) => Date.now() - lastActiveAt >= sessionIdleTimeoutMs;
    ...
    const getSessionUser = (request) => {
        const token = request.cookies?.[authCookies.access];
        if (!token)
            return null;
    ...
    const ok = (response, data, message, status = 200) => response.status(status).json({ success: true, message, data });
    ...
    app.post("/auth/refresh", (request, response) => {
        const token = request.cookies?.[authCookies.refresh];
    ...
    app.get("/menu", (request, response) => {
        const locale = request.query.locale?.toString() ?? defaultLocale;
    ...
    app.patch("/admin/menu-items/:id", requireAuth, requireRole("ADMIN"), (request, response) => {
        ...
        if (!item)
            return response.status(404).json({ success: false, message: "Menu item not found." });
    ...
    app.post("/admin/categories", requireAuth, requireRole("ADMIN"), (request, response) => {
        const created = { id: createId(), slug: request.body.slug, names: request.body.names, icon: request.body.icon || null, sortOrder: Number(request.body.sortOrder ?? 0) };
    ...
    app.post("/admin/users", requireAuth, requireRole("ADMIN"), (request, response) => {
        const created = { id: createId(), displayName: request.body.displayName, role: request.body.role, preferredLocale: request.body.preferredLocale || defaultLocale, pin: request.body.pin, isActive: true, createdAt: nowIso() };
    ...
    app.post("/admin/tables", requireAuth, requireRole("ADMIN"), (request, response) => {
        const created = { id: createId(), label: request.body.label, qrToken: createId(), isActive: true, createdAt: nowIso(), updatedAt: nowIso() };
