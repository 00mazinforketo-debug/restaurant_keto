...
import { createCategorySchema, createMenuItemSchema, createUserSchema, orderStatuses, socketEvents, updateOrderStatusSchema } from "@ros/shared";
import { z } from "zod";
...
import { recordOrderTransitionMetric } from "../lib/metrics.js";
...
const orderWhereFromQuery = (query) => {
    const status = typeof query.status === "string" && orderStatuses.includes(query.status)
        ? query.status
        : undefined;
        ...
        ...(search
            ? {
                OR: [
                    ...
                    { customerNameKu: { contains: search, mode: "insensitive" } },
                    { customerPhone: { contains: search } },
                    { customerAddressKu: { contains: search, mode: "insensitive" } }
                ]
            }
            : {})
    };
};
const getRevenueRangeSummary = async (startAt, endAt) => {
    const where = {
...
const getSummary = async () => {
    const now = dayjs();
    const [totalOrders, activeOrders, deliveredOrders, revenueToday, revenueWeek, revenueMonth, recentActivity] = await Promise.all([
        prisma.order.count(),
        prisma.order.count({
...
const emitOrderUpdate = async (request, orderId) => {
...
router.get("/orders", async (request, response, next) => {
    try {
        const orders = await prisma.order.findMany({
            where: orderWhereFromQuery(request.query),
