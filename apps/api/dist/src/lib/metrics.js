...
const dateKey = (input) => dayjs(input).startOf("day").toDate();
const upsertMetric = async (tx, date, changes) => {
...
export const recordOrderCreatedMetric = async (tx, placedAt) => upsertMetric(tx, placedAt, { totalOrders: 1, activeOrders: 1 });
export const recordOrderTransitionMetric = async (tx, status, totalPrice, at) => {
    if (status === OrderStatus.DELIVERED)
        return upsertMetric(tx, at, { activeOrders: -1, deliveredOrders: 1, revenue: Number(totalPrice) });
    return upsertMetric(tx, at, { activeOrders: -1, cancelledOrders: 1 });
};
export const rebuildMetrics = async (prisma) => {
