...
export const createMenuItemSchema = z.object({ slug: z.string().min(2).max(80), categoryId: z.string(), basePrice: z.number().nonnegative(), imageUrl: imageReferenceSchema.nullable().optional(), imagePublicId: z.string().nullable().optional(), isAvailable: z.boolean().default(true), sortOrder: z.number().int().nonnegative().default(0), translations: z.array(menuItemTranslationSchema).superRefine((translations, ctx) => { if (!translations.some((item) => item.locale === "ku"))
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Kurdish translation is required." }); }) });
...
export const apiResponseSchema = (data) => z.object({ success: z.boolean(), message: z.string().optional(), data });
